...
def _boltzmann_avg_groups_logz(
    ...
    out_cts, out_free_energy, out_logZ, out_avg_Eeff, out_cfg,
):
...
def evaluate_state_point(
    ...
    linear_term_mode="avgE", registry_mode="boltzmann",
    ...
    (
        eps_s, class_free_energy, logZ_internal, avg_Eeff, cube_to_species,
        m_patch, pts, pss, _cube_configs,
    ) = cubes_from_cache_fast(cache, J, mu_species, ws, registry_mode=registry_mode)

    A_row = np.sum(cube_to_species[:, :-1], axis=1) / 8.0

    mode = str(linear_term_mode).strip().lower()
    if mode in ("avge", "avg_e", "avg", "average", "average_energy", "averaged_energy"):
        ...
        raise ValueError(f"Unknown linear_term_mode={linear_term_mode!r}; use 'avgE' or 'logZ'.")
    ...
    if len(phis_solve) < 5:
        out = {
            ...
            "A_min": float(np.nanmin(A_row)), "A_max": float(np.nanmax(A_row)),
    ...
    if bethe_mode in ("selfconsistent_boundary_bethe", "self_consistent_boundary_bethe", "sc_boundary_bethe", "cavity_bethe"):
        ...
        if implicit_oriented_inputs is not None:
            ...
            # BETHE_CORRECTION is explicitly disallowed for oriented_directed_face_state above.
...
def cmd_merge_results(args):
    ...
    meta = {
        ...
        "factor": float(args.factor),
        "linear_term_mode": str(getattr(args, "linear_term_mode", "avgE")),
...
def add_grid_args(p):
    ...
    p.add_argument("--linear-term-mode", choices=("avgE", "logZ"), default="avgE",
                   help="e_linear used in reduced free energy: avgE gives the older Boltzmann-averaged Eeff; logZ uses -logZ_internal.")
