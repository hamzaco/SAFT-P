...
try:
    ...
    # The cache itself is loaded from disk; this import supplies only shared
    # geometry constants and the patch loader.  Keep the original import order
    # so adding the composition field does not silently change cache conventions.
    try:
...
class CubeWorkspace:
    ...
    def __init__(self, cache: dict, *, use_boundary_quotient: bool = True):
        ...
        # The oriented association solver expects an UNDIRECTED face-pattern
        # basis of size M**4 and adds the six laboratory directions exactly once.
        # Newer directional caches may already store 6*M**4 entries.  Feeding
        # that basis into the oriented solver would encode direction twice and
        # produce a spurious 36*M**4 association space.  Collapse such caches
        # back to their first M**4 pattern block; this changes no cube classes
        # or internal partition sums.
        raw_slots = np.asarray(cache["small_face_slots"], dtype=np.int64)
        n_patch_types = int(cache["n_patch_types"])
        face_radix = int(n_patch_types ** 4)
        raw_dirs = cache.get("small_face_dirs", None)
        self.cache_face_basis_was_directional = bool(
            raw_slots.shape[0] == 6 * face_radix and raw_dirs is not None
        )
        if self.cache_face_basis_was_directional:
            slots = np.ascontiguousarray(raw_slots[:face_radix], dtype=np.int64)
        else:
            slots = np.ascontiguousarray(raw_slots, dtype=np.int64)
        n_ft = len(slots)
        self.slots = slots
        self.n_ft = n_ft
...
def evaluate_state_point(
    ...
    # For the present particle-vacancy cube models, the full outward boundary
    # signature identifies whether each corner is occupied because the vacancy
    # patch type is distinct.  Consequently every canonical boundary class must
    # have exactly one occupied-particle sector.  In that case
    #   Z_g(nu) = exp(8*nu*A_g) Z_g(0),
    # so the exact-nu formulation is algebraically identical to the fixed-class
    # logZ formulation.  This is a strong invariant and catches cache/basis bugs.
    finite_occ = np.isfinite(logZ_by_nocc)
    n_finite_occ = np.sum(finite_occ, axis=1)
    fixed_occupancy_classes = bool(np.all(n_finite_occ == 1))
    fixed_occ_sector = None
    fixed_occ_A_error = np.nan
    fixed_occ_logZ_error = np.nan
    if fixed_occupancy_classes:
        fixed_occ_sector = np.argmax(finite_occ, axis=1).astype(np.int64)
        A_from_sector = fixed_occ_sector.astype(np.float64) / 8.0
        selected_logZ = logZ_by_nocc[np.arange(logZ_by_nocc.shape[0]), fixed_occ_sector]
        fixed_occ_A_error = float(np.max(np.abs(A_row - A_from_sector)))
        fixed_occ_logZ_error = float(np.max(np.abs(logZ_internal - selected_logZ)))
        if fixed_occ_A_error > 1e-10 or fixed_occ_logZ_error > 1e-10:
            raise RuntimeError(
                "Fixed-occupancy class invariant failed: "
                f"max|A-A_sector|={fixed_occ_A_error:.3e}, "
                f"max|logZ-logZ_sector|={fixed_occ_logZ_error:.3e}. "
                "The cache or class reduction is inconsistent."
            )
    ...
    if mode in ("logznu", "logz_nu", "nu", "exact_nu", "class_free_energy_nu"):
        ...
        if fixed_occupancy_classes:
            # Exact factorization for the current particle-vacancy classes.
            # Route through the established fixed-class solver so logZnu and
            # logZ are numerically identical, as they must be.
            A_row = fixed_occ_sector.astype(np.float64) / 8.0
            linear_term_mode_canonical = "logZnu_factorized"
            exact_internal_nu = False
        else:
            # General case: classes genuinely mix composition sectors, so nu
            # must remain inside the internal partition function.
    ...
    if ws.cache_face_basis_was_directional and association_mode_canonical in (
        "global", "global_face", "standard", "saftp", "original",
        "directed_face", "directed_face_state", "state", "state_assoc", "x_gs", "x_gd", "direction_a",
    ):
        raise ValueError(
            "This cache stores a direction-resolved 6*M**4 face basis. "
            "Use association_mode=oriented_directed_face_state with this scanner; "
            "the scanner collapses to M**4 patterns and adds direction once. "
            "Using global/direct modes here would mix incompatible cache conventions."
        )
    ...
    if len(phis_solve) < 5:
        out = {
            ...
            "fixed_occupancy_classes": fixed_occupancy_classes,
            "fixed_occ_A_error": fixed_occ_A_error,
            "fixed_occ_logZ_error": fixed_occ_logZ_error,
            "cache_face_basis_was_directional": ws.cache_face_basis_was_directional,
